...
async def check_ollama_connection():
    """
    FastAPI 서버 시작 시 Ollama 서버가 켜져 있는지 확인합니다.
    """
    try:
        print("[Ollama Service] 🟡 Ollama 서버 연결을 시도합니다...")
        # 간단한 list 요청으로 서버 응답 확인
        await _client.get("http://localhost:11434/api/tags")
        ...
        print("[Ollama Service] 🔴 Ollama가 로컬에서 실행 중인지 확인하세요. (e.g., 'ollama serve')", file=sys.stderr)
    except Exception as e:
        print(f"[Ollama Service] 🔴 Ollama 연결 중 알 수 없는 오류: {e}", file=sys.stderr)
...
async def get_summary(transcript: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[Ollama Service] 🔴 Ollama API 오류 (HTTP {e.response.status_code}): {e.response.text}", file=sys.stderr)
